...
1. Laptop 의 기본 성능과 가격을 분석하여 데이터를 비교
  
2. 사용자가 원하는 노트북 옵션을 선택하면 해당 옵션 기능에 해당되는 노트북 가격을 예측,  
  
Scikit-Learn 의 OneHotEncoder 라이브러리를 이용하여 문자열 데이터를 수치 데이터로 바꾼 후,  
LinearRegression 인공지능을 학습시켜 노트북 가격을 예측할 수 있는 인공지능 모델 개발  
joblib 라이브러리로 개발된 모델을 pkl 파일로 저장하여 유저가 새로운 데이터를 입력하였을 때 그 결과값을 예측하도록 함  
  
3. 또는 유저가 옵션을 선택하면 옵션에 해당되는 제품을 나타내도록 함.  

<br/>


### 📌 AWS EC2 배포, Github Actions 이용 (CI/CD 구축)

AWS 의 EC2 서버를 이용하여서, 웹 대시보드를 서버에 배포 하였고,
  
Github Actions 기능을 이용하여서, 코드 수정사항을 Github 에 push 할 때마다  
putty 에 접속하지 않아도, 자동으로 배포할 수 있도록 함.  
